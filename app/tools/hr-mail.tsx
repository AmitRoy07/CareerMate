...
import type { GenerateHrMailResult, HrMailTone, HrMailType } from '@/types/hr-mail.types';
...
export default function HrMailScreen() {
  ...
  async function handleGenerate() {
    ...
    try {
      setLoading(true);
      setError(null);
      const nextResult = await generateHrMail({ userId: user?.id, mailType, tone, name, companyName, role, hrOrManagerName });
      setResult(nextResult);
      await trackEvent('hr_mail_generated', { mailType });
    } catch (currentError) {
      setError(currentError instanceof Error ? currentError.message : 'Unable to generate HR mail right now. Please try again.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Screen>
      <View className="gap-2">
        ...
        <Text variant="label" style={{ color: colors.primary }}>Premium Tool</Text>
        <Text variant="title">HR Mail Generator</Text>
        <Text variant="muted">Generate professional HR emails and short messages without aggressive or false claims.</Text>
      </View>
      ...
      <Card>
        ...
        <View className="flex-row flex-wrap gap-2">
          {mailTypes.map((item) => (
            <Chip key={item.value} label={item.label} active={mailType === item.value} onPress={() => setMailType(item.value)} />
          ))}
        </View>
      </Card>
      ...
      <Card>
        ...
        <Field value={companyName} onChangeText={setCompanyName} placeholder="Company name" />
        <Field value={role} onChangeText={setRole} placeholder="Role" />
        <Field value={hrOrManagerName} onChangeText={setHrOrManagerName} placeholder="HR/Manager name optional" />
        <View className="flex-row flex-wrap gap-2">
          {tones.map((item) => <Chip key={item} label={item} active={tone === item} onPress={() => setTone(item)} />)}
        </View>
        <Button title={loading ? 'Generating...' : 'Generate Mail'} icon={MailPlus} loading={loading} onPress={handleGenerate} />
      </Card>

      {error ? (
        <UpgradePrompt
          title="Premium access needed"
          message={error.includes('requires') || error.includes('credits') ? buildPaywallMessage({ entitlement: 'hr_mail_generator', creditType: 'hr_mail', featureName: 'HR Mail Generator' }) : error}
      ...
      {result ? (
        <>
          ...
          <Button title="Save generated mail" icon={Save} variant="secondary" onPress={handleSave} />
        </>
      ) : null}
    </Screen>
  );
...
function Chip({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  ...
  const colors = Colors[colorScheme];
  return (
    <Pressable onPress={onPress} style={{ backgroundColor: active ? colors.primary : colors.surfaceLow, borderColor: active ? colors.primary : colors.border, borderRadius: 999, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 8 }}>
      <Text style={{ color: active ? '#FFFFFF' : colors.text, fontFamily: 'PlusJakartaSans_700Bold' }}>{label}</Text>
    </Pressable>
  );
}
